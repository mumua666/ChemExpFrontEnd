...
export default {
  ...
  methods: {
    ...
    computeMatrix() {
      ...
      if (angle == 270) {
        let x = this.real.width - oy
        let y = ox
        ox = x
        oy = y
      }
      if (angle == 180) {
        let x = this.real.width - ox
        let y = this.real.height - oy
        ox = x
        oy = y
      }
      if (angle == 90) {
        let x = oy
        let y = this.real.height - ox
        ox = x
        oy = y
      }
      sx = ox - sw / 2
    ...
    parsePoint(point, angle) {
      ...
      result.y =
        point.y * Math.cos((angle * Math.PI) / 180) + point.x * Math.sin((angle * Math.PI) / 180)
      return result
    },
