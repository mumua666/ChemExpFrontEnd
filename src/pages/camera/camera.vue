<script setup lang="ts">
	...
	const takePhoto = async () => {
		...
		uni.chooseVideo({
			...
			// sourceType: '拍摄',
			success: (res) => {
				photoSrc.value = res.tempFilePath
			},
			fail: (err) => {
				// #ifdef MP
				uni.getSetting({
...
<template>
	<view v-if="!photoSrc" class="container">
		<view v-if="photoSrcs.length" class="gallery">
			<image v-for="(photoSrc, index) in photoSrcs" :key="index" :src="photoSrc" class="gallery-image"
				@click="previewImage(photoSrc)"></image>
		</view>
		<template>
			<camera id="camera" device-position="back" flash="auto" style="width: 100%; height: 100vh"></camera>
			<view class="overlay">
	...
	<template v-else>
		<ksp-cropper mode="fixed" :width="300" :height="400" :maxWidth="1024" :maxHeight="1024" :url="photoSrc"
